...
start_containers() {
    ...
    if grep -q "NEW_INSTALL=true" .env; then
        print_message "New installation detected ..."
        if [ -f "../app/init.sql" ]; then
            print_message "init.sql file already exists."
            # check if init.sql is not empty
            if [ ! -s "../app/init.sql" ]; then
                print_message "Error: I can't start. Please choose menu 1 to configure the application."
                exit 1
            fi
        else
            print_message "Error: I can't start. Please choose menu 1 to configure the application."
...
generate_init_sql() {
    ...
    # Ask for the new root password for MySQL
    print_message "You now need to define a new password for the 'root' user of the database."
    print_message "Please save it securely in your password manager."
    while true; do
...
EOF
    ...
    sed -i "s/\${root_password}/${root_password}/g" ../app/init.sql
